...
event-free state to a second state representing death from any cause.
...
\emph{Kaplan--Meier} or \emph{product-limit} estimator {[}citation
here{]}. For a set of \(N\) ordered event times
...
\] assuming the distribution of \(T\) is continuous, and \(H_k(t)\) is
the cause-specific cumulative hazard function for the \(k^{\text{th}}\)
event. The cause-specific cumulative incidence function is then defined
as \[
...
{\centering \includegraphics{general-introduction_files/figure-pdf/unnamed-chunk-2-1.pdf}

}

\caption{Adapted from Figure 2.1 in Geskus}
