...
of particular genetic mutations, influence the rate of death over time.
...
crucial in determining what kind of methodology we can use, and the
potential consequences of using naive approaches.
...
modelling the all-cause hazard we can also estimate the overall survival
function. This is useful since by assuming that \(\tilde{T}\) and \(C\)
are stochastically independent of each other, the hazard function
remains ``undisturbed'' by any right-censoring
...
(\protect\hyperlink{ref-kalbfleisch2011statistical}{2011}).
...
summarises two key aspects of the survival function at a particular
timepoint: its slope (i.e.~how rapidly the curve is decreasing,
expressed as a rate per unit time), and the proportion of individuals
still alive.}
...
also Little
(\protect\hyperlink{ref-littleMissingDataAssumptions2021}{2021}) for an
up-to-date discussion on missing data assumptions.
...
(\protect\hyperlink{ref-mengMultipleImputationInferencesUncongenial1994}{Meng,
...
\begin{equation}\protect\hypertarget{eq-current-value}{}{
h_i(t \given \mathcal{M}_i(t),\mathbf{w}_i) = h_0(t)\exp\{\boldsymbol{\gamma}^\intercal\mathbf{w}_i + \alpha m_i(t)\},
}\label{eq-current-value}\end{equation} where
\(\mathcal{M}_i(t) = \{m_i(s), \ 0 \leq s < t\}\) represents the history
of the true marker value up to time \(t\), and \(\mathbf{w}_i\) is a
vector of baseline covariates. The association between the true
``current value'' of the longitudinal marker and the terminating event
is quantified by \(\alpha\), and the baseline hazard \(h_0(t)\) is often
assumed to follow a flexible parametric form (e.g.~using a B-spline
basis).
...
as exogenous
(\protect\hyperlink{ref-kalbfleisch2011statistical}{Kalbfleisch and
Prentice, 2011};
\protect\hyperlink{ref-rizopoulosJointModelsLongitudinal2012}{Rizopoulos,
2012};
\protect\hyperlink{ref-sweetingJointModellingLongitudinal2011}{Sweeting
and Thompson, 2011}).
...
(\protect\hyperlink{ref-hickeyJointModellingTimetoevent2016}{Hickey
\emph{et al.}, 2016}). Furthermore, Papageorgiou \emph{et al.}
(\protect\hyperlink{ref-papageorgiouOverviewJointModeling2019a}{2019})
outline various extensions for the association structure between the
...
al.} (\protect\hyperlink{ref-hickeyComparisonJointModels2018a}{2018})
provides an overview of joint models that consider time to multiple
competing events. In Chapter~\ref{sec-chap-JM-DLI} of this thesis, we
use the competing risks joint model described in Rizopoulos
(\protect\hyperlink{ref-rizopoulosJointModelsLongitudinal2012}{2012})
(based on cause-specific Cox proportional hazards) to model the
...
being increasingly used (e.g. Hayati Rezvan \emph{et al.}
(\protect\hyperlink{ref-hayatirezvanRiseMultipleImputation2015}{2015})),
we do not know whether this is also the case for alloSCT studies, or if
the methodology is being applied and reported adequately. This chapter
is based on the following publication:

\begin{quote}
\textbf{Bonneville, E. F.}, Schetelig, J., Putter, H., et al.~(2023)
Handling missing covariate data in clinical studies in haematology.
\emph{Best Practice \& Research Clinical Haematology}, 36, 101477. DOI:
...
and Taylor, 2016}) as part of large, fully-factorial simulation study. A
novel aspect of this simulation study is the focus on the impact of
missing covariate data on the estimation of individual-specific
cumulative incidence functions. Furthermore, the methodology is applied
on a dataset with long-term follow-up after an alloSCT for patients with
myelodysplastic syndromes or secondary acute myeloid leukaemia
(\protect\hyperlink{ref-scheteligLateTreatmentrelatedMortality2019}{Schetelig
\emph{et al.}, 2019}). This chapter is based on:

\begin{quote}
\textbf{Bonneville, E. F.}, Resche-Rigon, M., Schetelig, J., et
al.~(2022) Multiple imputation for cause-specific Cox models: Assessing
methods for estimation and prediction. \emph{Statistical Methods in
Medical Research}, 31, 1860--1880. SAGE Publications Ltd STM. DOI:
...
imputing missing covariates compatibly with a Fine--Gray analysis model
for a competing event of interest, without needing to specify a model
for the competing event(s). As mentioned by Lau and Lesko
(\protect\hyperlink{ref-lauMissingnessSettingCompeting2018}{2018}),
there is a paucity of research regarding the use of multiple imputation
for missing covariates in competing risks settings, and even more so in
the context of subdistribution hazard models. The simulation study in
this work is, to the best of our knowledge, the first systematic
assessment of how substantive model misspecification at the covariate
imputation stage affects inference in the competing risks setting. The
methodology is also assessed using a dataset of patients with
myelofibrosis who has undergone an alloSCT
(\protect\hyperlink{ref-polverelliImpactComorbiditiesBody2024a}{Polverelli
\emph{et al.}, 2024}). This chapter is based on the following preprint
(and is under review in \emph{Statistics in Medicine}):

\begin{quote}
\textbf{Bonneville, E. F.}, Beyersmann, J., Keogh, R. H., et al.~(2024)
Multiple imputation of missing covariates when using the Fine--Gray
model. arXiv:2405.16602. arXiv. DOI:
...
Chapter~\ref{sec-chap-FG-MI}, which is based on the subdistribution time
for only one competing event). In isolation however, this article
provides simulation-based arguments against specifying Fine--Gray model
for more than one competing event, and using cause-specific hazard
models instead. This chapter is based on the following publication:
...
to this work, joint models were rarely used in the alloSCT context (to
the best of our knowledge, only in Salzmann-Manrique \emph{et al.}
(\protect\hyperlink{ref-salzmann-manriqueJointModelingImmune2018a}{2018})),
and did not consider competing risks in the time-to-event submodel or
time-dependent interventions such as a donor lymphocyte infusion. This
chapter is based on:

\begin{quote}
Koster, E. A. S., \textbf{Bonneville, E. F.}, Borne, P. A. von dem, et
al.~(2023) Joint models quantify associations between immune cell
...
\emph{Frontiers in Immunology}, 14. Frontiers. DOI:
...
\hypertarget{other-ideas-and-notes}{%
\section*{Other ideas and notes}\label{other-ideas-and-notes}}
\addcontentsline{toc}{section}{Other ideas and notes}

\markright{Other ideas and notes}
...
\item
  use \(f(\cdot)\) for density also in cause-spec. paper
\item
...
\hypertarget{sec-chap-haema-review}{%
...
\hypertarget{sec-chap-discussion}{%
\chapter{General Discussion}\label{sec-chap-discussion}}
...
\leavevmode\vadjust pre{\hypertarget{ref-hickeyJointModellingTimetoevent2016}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-littleMissingDataAssumptions2021}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-mengMultipleImputationInferencesUncongenial1994}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-papageorgiouOverviewJointModeling2019a}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-sweetingJointModellingLongitudinal2011}{}}%
