...
function of covariates, one will usually need to specify regression
models the cause-specific hazards, which are identifiable and estimable
from observed data
...
\(\mathbf{Z}\) is a \(p\)-dimensional vector of covariates, and
\(\boldsymbol{\beta}_k\) quantify the impact of \(\mathbf{Z}\) on the
...
described this core issue as follows
...
we know that an individual is event-free by their censoring time. When
data are missing on covariates, we are usually less fortunate, as these
will tend to be fully missing.
...
in a least one of multiple relevant covariates can result in analyses
that biased and/or inefficient.
...
\item
  Cornfield
  (\protect\hyperlink{ref-cornfieldEstimationProbabilityDeveloping1957}{1957})
  to cite re. definition of competing risks. Also nice: ``\,``re. mixed
  and pure probabilities, sames as net/crude mortality. Also the fun
  quote as the end of the paper!
\item
  Prentice \emph{et al.}
...
\leavevmode\vadjust pre{\hypertarget{ref-chiang1961probability}{}}%
...
\leavevmode\vadjust pre{\hypertarget{ref-orchard1972missing}{}}%
