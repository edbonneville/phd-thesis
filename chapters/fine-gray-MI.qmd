...
# Multiple imputation of missing covariates when using the Fine--Gray model {#sec-chap-FG-MI}
...
\blfootnote{Chapter based on: \textbf{Bonneville, E. F.}, Beyersmann, J., Keogh, R. H., et al. (2024) Multiple imputation of missing covariates when using the Fine--Gray model. arXiv:2405.16602. arXiv. DOI: \href{https://doi.org/10.48550/arXiv.2405.16602}{10.48550/arXiv.2405.16602}.}
...
\begin{equation*}
	\lambda_1(t \given \mathbf{Z}) = \lambda_{01}(t)\exp(\boldsymbol{\beta}^\top \mathbf{Z}),
\end{equation*}
...
\begin{equation*}
	F_1(t \given \mathbf{Z}) = 1 - \exp \Biggl\{ -\exp(\boldsymbol{\beta}^\top \mathbf{Z}) \int_{0}^{t} \lambda_{01}(u)\diff u \Biggr\},
\end{equation*}
where $\int_{0}^{t} \lambda_{01}(u)\diff u = \Lambda_{01}(t)$ is the cumulative baseline subdistribution hazard. If we define a baseline cumulative incidence function $F_{01}(t) = 1 - \exp\{-\Lambda_{01}(t)\}$ (i.e. the cumulative incidence when $\mathbf{Z} = 0$), the model can also be written as
$$
F_1(t \given \mathbf{Z}) = 1 - \{1 - F_{01}(t)\}^{\exp(\boldsymbol{\beta}^\top \mathbf{Z})}.
$${#eq-cuminc-comp}
...
The substantive model of interest is $\lambda_1(t \given X, Z) = \lambda_{01}(t)\exp\{g(X,Z;\boldsymbol{\beta})\}$, which is a Fine--Gray model for cause 1, and where $g(X,Z;\boldsymbol{\beta})$ is a function of $X$ and $Z$, parametrised by $\boldsymbol{\beta}$. In this section, we provide an overview of possible approaches for imputing each partially observed $X_j$. These imputation models can then be 'chained' together as described in Sections 4 and 5 of the work by @bartlettMultipleImputationCovariates2015. In addition to an approach which imputes compatibly with the assumed substantive model, we also consider alternative methods which are either a) only approximately compatible with the substantive model; or b) impute assuming a different underlying competing risks structure (i.e. cause-specific proportional hazards). We require that the proposed approaches be valid under the missing-at-random (MAR) assumption, that is, $P(R \given T, D, X, Z) = P(R \given T, D, X^{\text{obs}}, Z)$. 
...
A relevant question at this point is whether the relation between cause-specific and subdistribution hazards can instead be used as part of a procedure to impute compatibly with proportional subdistribution hazards for cause 1. In order to motivate such a procedure, we first note that the conditional density of the observed outcome given covariates used in @eq-imput-dens can be written both in terms of cause-specific hazards, and in terms of the cumulative incidence functions, as
$$
\begin{aligned}
...
\end{aligned}
$${#eq-subdenstosubdist}
where $\Lambda_k(t \given X, Z)$ is the cumulative subdistribution hazard for cause $k$ conditional on $X$ and $Z$. Specifying a Fine--Gray model for cause 1 is an assumption regarding only part of @eq-outcome-dens, namely for any terms involving $f_1(T \given X, Z)$. The practical implication of this is that @eq-imput-dens cannot be used to impute the missing $X_j$ without making assumptions about cause 2. One could for example assume (for imputation purposes) a cause-specific Cox model for cause 2, derive the implied $h_1(t \given X, Z)$ using the relation between the subdistribution hazard and the cause-specific hazards, and then use both cause-specific hazards to evaluate $f(T, D \given X, Z)$ in @eq-imput-dens. 

Given that a Fine--Gray model is assumed for cause 1, some computational difficulties can be encountered while making assumptions for cause 2. For example, specifying a Fine--Gray model also for cause 2 in the imputation procedure could result in the total failure probability at an observed event time $F_1(T \given X, Z) + F_2(T \given X, Z)$ exceeding 1, meaning we would not be able to draw imputed values using @eq-imput-dens for high-risk individuals [@austinFineGraySubdistributionHazard2021]. An additional example concerns the approach described in the previous paragraph, where $h_1(t \given X, Z)$ is derived based on $h_2(t \given X, Z)$ and $\lambda_1(t \given X, Z)$. The numerical integration step generally needed to compute $h_1(t \given X, Z)$ could make the overall imputation procedure rather computationally inefficient. More details on potential issues when specifying a model for cause 2 when a Fine--Gray model is assumed for cause 1 can be found in @bonnevilleWhyYouShould2024a 
...
One option to sample from $f(X^{\text{mis}}, V^{\text{mis}} \given X^{\text{obs}},Z,V^{\text{obs}},D)$, the joint posterior predictive density, is to use a sequential approach, where we factorise
\begin{align*}
...
\end{align*} 
The above is valid as long as $C \indep X \given Z$. Practically speaking, this involves imputing the potential censoring times (possibly in strata of $Z$) in a first step, and then imputing the missing $X$ in a second step. This can be implemented easily using existing software packages in R: {kmi} for the imputation of censoring times [@allignolSoftwareFittingNonstandard2010], and {smcfcs} for the imputation of the missing covariates [@bartlettSmcfcsMultipleImputation2022]---see @fig-workflow for an illustration of the workflow.

```{r}
#| label: fig-workflow
#| fig-cap: "Sequential workflow for (compatible) covariate
#| imputation and analysis for a
#| Fine--Gray substantive model with two covariates $X$ and
#| $Z$, in
#| the presence of random right-censoring. In the first step,
#| the potential censoring times for those failing from
#| cause 2 are multiple imputed using the {kmi} package. In the
#| second step, the missing covariates are imputed using the
#| {smcfcs} (or {mice}) package. This workflow is valid
#| when the probability of being censored is independent of $X$
#| and any $Z$ related to the censoring process are modelled in
#| {kmi}."
#| echo: false
#| out-width: "100%"

knitr::include_graphics(here::here(paste0("figures/fine-gray-MI_workflow2", extension)))
```

If the censoring process additionally depends on the partially observed $X$, we will need to iteratively sample from $f(V^{\text{mis}} \given X,Z,V^{\text{obs}},D)$ and $f(X^{\text{mis}} \given X^{\text{obs}},Z,V,D)$ with the following modifications:
...
## Simulation study {#sec-sim-study}
...
#### Covariate missingness {#sec-simstudy-missings}
...
- CS-SMC: MI, imputing compatibly with cause-specific Cox proportional hazards models. This method is described in @sec-cs-smc. Both $X$ and $Z$ are used as predictors in each cause-specific model assumed by this procedure. 
- CS-Approx: MI with both marginal cumulative cause-specific hazards (evaluated at the individual observed event or censoring time) and competing event indicator included as predictors in the imputation model, in addition to $Z$. This method is described in @sec-cs-approx.
- FG-SMC: MI, imputing compatibly with a Fine--Gray model for cause 1 that has as covariates $X$ and $Z$. This is the method described in @sec-fg-smc.
- FG-Approx: MI with marginal cumulative subdistribution hazard (evaluated at the individual observed or imputed subdistribution time $V$) and indicator for event 1 included as predictors in the imputation model, in addition to $Z$. This method is described in @sec-fg-approx. 
...
Analyses were performed using R version 4.3.1 [@rcoreteamLanguageEnvironmentStatistical2023]. Core packages used were: {survival} version 3.5.7 [@survival-package], {mice} version 3.16.0 [@vanbuurenMiceMultivariateImputation2011], {smcfcs} version 1.7.1 [@bartlettSmcfcsMultipleImputation2022], {kmi} version 0.5.5 [@allignolSoftwareFittingNonstandard2010], and {rsimsum} version 0.11.3 [@gaspariniRsimsumSummariseResults2018].
...
Supplementary materials for this work are available at [https://arxiv.org/abs/2405.16602](https://arxiv.org/abs/2405.16602).
...
## Appendix A: Imputed censoring times, and resulting cumulative subdistribution hazards {.unnumbered}

As described in Section @sec-imp-cens, the subdistribution time $V$ is only partially observed in the presence of random right-censoring. Thus, the potential censoring times for those failing from cause 2 should first be multiply imputed, before imputing any missing covariates. This imputation of partially observed $V$ is visualised more closely in @fig-cens-appendix, using a simulated dataset of 2000 individuals following the parametrisation used in the simulation study scenario with correctly specified Fine--Gray, $p = 0.65$, and random exponential censoring. In this example, the potential censoring times for those failing from cause 2 were imputed $m = 10$ times.

```{r}
#| label: fig-cens-appendix
#| fig-cap: "Based on a simulated dataset of $n = 2000$ (correctly specified Fine--Gray, $p = 0.65$, random censoring), we show the imputed ($m = 10$ imputations) potential censoring times for a random selection of 20 individuals failing from cause 2 (upper panel); and the estimated marginal cumulative subdistribution hazard function for cause 1 based on true $V$, and based on imputed $V$ (lower panel)."
#| echo: false
#| out-width: "100%"

knitr::include_graphics(here::here(paste0("figures/fine-gray-MI_cens-appendix", extension)))
```
...
## Appendix B: Additional simulations {.unnumbered}
...
knitr::include_graphics(here::here(paste0("figures/fine-gray-MI_mart-sims", extension)))
```

```{r}
#| label: fig-bias-covar-cens
#| fig-cap: |
#|    Relative bias (%) in estimating $\{\beta_1,\beta_2\}=\{0.75,0.5\}$, with
#|    corresponding 95% Monte Carlo confidence interval (constructed using the
#|    standard normal approximation). These are additional simulations under
...
knitr::include_graphics(here::here(paste0("figures/fine-gray-MI_covar-cens-sims", extension)))
```
...
#| warning: false
#| message: false
#| label: tbl-data-dict-mf
#| tbl-cap: "Data dictionary. The median and interquartilerange are reported for continuous variables, and for categorical variables the counts and proportion per level are reported. The difference between this cohort (n = 3982) and the one reported in @sec-chap-polverelli (n = 4086), is that the present cohort is selected on available outcome data (i.e. no missing relapse times). CMV: cytomegalovirus; HLA: human leukocyte antigen; HCT-CI: Hematopoietic stem cell transplantation-comorbidity index; MF: myelofibrosis."

# (See the haema review example!)

dat_data_dict <- read.csv(
  file = here::here("tables/MI-FG_data-dictionary.csv"), 
  sep = ";",
  header = TRUE
)
colnames(dat_data_dict) <- c("Variable or level", "N = 3982")

dat_data_dict$`N = 3982` <- ifelse(
    is.na(dat_data_dict$`N = 3982`),
    "",
    dat_data_dict$`N = 3982`
  )
  
contin_vars <- c(
  "White blood cell count (WBC, x$10^9$/L)",
  "Peripheral blood (PB) blasts (%)",
  "Interval diagnosis-transplantation (years)",
...
if (knitr::is_html_output()) {
  dat_data_dict$`Variable or level` <- ifelse(
   (dat_data_dict$`Variable or level` %in% contin_vars) |
     (dat_data_dict$`N = 3982` == ""),
   no = paste0("&nbsp;&nbsp;&nbsp;&nbsp;", dat_data_dict$`Variable or level`),
   yes = dat_data_dict$`Variable or level`
  )
  dat_data_dict$`Variable or level` <- gsub(
    pattern = "(\\$\\\\geq)",
    x = dat_data_dict$`Variable or level`, 
    replacement = "&ge; "
  )
  dat_data_dict$`Variable or level` <- gsub(
    pattern = "(\\$\\\\leq)",
  ...
  kbl(dat_data_dict, escape = FALSE)
} else {
  ...
  dat_data_dict$`Variable or level` <- ifelse(
   (dat_data_dict$`Variable or level` %in% contin_vars) |
     (dat_data_dict$`N = 3982` == ""),
   #no = paste0("&nbsp;", dat_data_dict$`Variable or level`),
   no = paste0("\\quad ", dat_data_dict$`Variable or level`),
   yes = dat_data_dict$`Variable or level`
  )
  
  dat_data_dict$`N = 3982` <- gsub(
    pattern = "(\\%)",
    x = dat_data_dict$`N = 3982`, 
    replacement = "\\\\%"
  )
  dat_data_dict$`Variable or level` <- gsub(
    pattern = "(\\%)",
