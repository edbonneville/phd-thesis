...
If $Z$ is continuous, there are no exact results due to the $\exp(\gamma_k Z)$ terms. Analogously to White and Royston, we use a first order Taylor series around $\bar{Z}$ to approximate it as $\exp(\gamma_k Z) \approx \exp(\gamma_k \bar{Z}) [ 1 + \gamma_k(Z - \bar{Z})  ]$, which is valid if $\Var(\gamma_k Z)$ is small.\cite{whiteImputingMissingCovariate2009} We can thus write
\begin{align*}
